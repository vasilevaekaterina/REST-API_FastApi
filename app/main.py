...
def search_advertisements(
    ...
    price_min: float | None = Query(None, gt=0, description="Мин. цена"),
    price_max: float | None = Query(None, gt=0, description="Макс. цена"),
    author: str | None = Query(None, description="Поиск по автору"),
    offset: int = Query(0, ge=0, description="Смещение выборки"),
    limit: int | None = Query(
        None,
        gt=0,
        description="Максимальное количество объявлений",
    ),
